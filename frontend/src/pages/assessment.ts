...
function getQuizHTML() {
  const qHtml = state.questions.map((q, i) => {
    ...
    const opts = (q.options || []).map((opt: string, idx: number) => {
      ...
      return `<button class="opt-btn btn-outline ${isSel ? 'selected' : ''}" data-qi="${i}" data-val="${letter}" style="display: block; width: 100%; text-align: left; margin-bottom: 8px; border-radius: 8px;">${opt}</button>`;
    }).join('');
    return `
