    ...
    <div class="fade-in" style="max-width: 680px; margin: 40px auto;">
      ...
      <div class="glass-card">
        ...
        <button id="rj-submit" class="btn-primary" style="width: 100%;" ${state.loading ? 'disabled' : ''}>
          ${state.loading ? 'Analyzing... (may take 1-3 min)' : 'Analyze Repository'}
        </button>
        
        ${state.loading ? `
          <div class="alert-info" style="margin-top: 24px;">
            <strong>${state.loadingStep || 'Working...'}</strong><br>
            <span style="font-size: 0.8rem; color: var(--text-muted);">This can take 2-5 minutes for large repos — please keep this tab open!</span>
          </div>
        ` : ''}
      </div>
    </div>
...
function getResultsHTML() {
  ...
  const renderScoreBar = (label: string, score: number, scColor: string) => `
    ...
    <div class="fade-in" style="max-width: 860px; margin: 40px auto; padding-bottom: 100px;">
      ...
      <div class="glass-card" style="margin-bottom: 32px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 16px;">
          ${renderScoreBar('Functionality', r.scores?.functionality?.score || 0, '#00ffcc')}
          ${renderScoreBar('Code Quality', r.scores?.code_quality?.score || 0, '#0088ff')}
          ${renderScoreBar('Documentation', r.scores?.documentation?.score || 0, '#00ff66')}
          ${renderScoreBar('Architecture', r.scores?.architecture?.score || 0, '#f59e0b')}
        </div>
      </div>
      ...
      ${r.mentor_notes ? `
        <div class="glass-card" style="margin-bottom: 32px; border-left: 4px solid #4f46e5;">
          ...
          <p style="font-style: italic;">${r.mentor_notes}</p>
        </div>
      ` : ''}
