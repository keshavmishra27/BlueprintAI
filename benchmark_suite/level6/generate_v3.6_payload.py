...
def create_payload():
    payload = {
        ...
        "candidate_uncertainties": [
            {
                "id": "unc-001",
                "question_text": "Can the hospital's existing clinical information system expose the required operational data through an approved interface with sufficient freshness and access privileges?",
                "question_target": "Data Availability & Freshness",
                "unknown_fact": "Interface Availability and Policy",
                "importance": "CRITICAL",
                "yes_mutation": {
                    "add_constraints": ["approved_hl7_interface_available"],
                    "remove_constraints": []
                },
                "no_mutation": {
                    "add_constraints": ["no_approved_interface_available"],
                    "remove_constraints": []
                },
                "yes_candidate_architecture": {
                    "inputs": ["real-time clinical data feed"],
                    "processing": ["local predictive model"],
                    "decision": ["overcrowding risk assessment"],
                    "output": ["local hospital dashboard"],
                    "capabilities": ["predict_patient_waiting_times", "identify_overcrowding_early"],
                    "data_required": ["historical queue data", "live arrival data"],
                    "resources_required": ["existing hospital computer"],
                    "constraints": ["approved_hl7_interface_available"],
                    "evidence_provenance": [],
                    "historical_decisions": [],
                    "semantic_dependencies": [
                        "requires_historical_data_access",
                ...
                "no_candidate_architecture": {
                    ...
                    "constraints": ["no_approved_interface_available"],
                    "evidence_provenance": [],
                    "historical_decisions": [],
                    "semantic_dependencies": [
                        "requires_historical_data_access",
    ...
    out_dir = os.path.join(os.path.dirname(__file__), "results", "v3.6_hospital_case")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "raw_refiner_output.json")
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2)
        
    print(f"Generated V3.6 payload at {out_path}")
