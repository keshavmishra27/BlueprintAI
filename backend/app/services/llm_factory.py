...
def check_llm_availability():
    ...
    if groq_key and groq_key != "your_groq_api_key_here":
        logger.info("Cloud LLM (Groq) detected via GROQ_API_KEY.")
        return
...
GROQ_CREW_MODEL = os.getenv("GROQ_CREW_MODEL", GROQ_FALLBACK_MODEL)
GROQ_FALLBACK_MAX_TOKENS = int(os.getenv("GROQ_FALLBACK_MAX_TOKENS", "4096"))
GROQ_CREW_MAX_TOKENS = int(os.getenv("GROQ_CREW_MAX_TOKENS", "8192"))
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", "llama3.2")
OLLAMA_BASE_URL = os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
LLM_MAX_TOKENS = int(os.getenv("LLM_MAX_TOKENS", "4096"))
LLM_TIMEOUT = int(os.getenv("LLM_TIMEOUT", "60"))
logger.info("LLM Factory initialized (Groq=%s, max_tokens=%s, crew_max_tokens=%s)", "yes" if GROQ_API_KEY else "no", LLM_MAX_TOKENS, GROQ_CREW_MAX_TOKENS)
        ...
        if end != -1 and end > start:
            ...
            try:
                ...
                try:
                    ...
                    pass
    return {}
def _parse_groq_available_tokens(error: Exception) -> int | None:
    if not error:
        return None
    text = str(error)
...
def get_hybrid_llm(temperature=0.7):
    ...
    if has_key:
        try:
            ...
            llms.append(xai)
            logger.info("Groq primary model added to LLM list")
        except Exception as e:
            logger.warning(f"Primary Groq model {GROQ_MODEL} failed to initialize: {e}")
        if GROQ_FALLBACK_MODEL and GROQ_FALLBACK_MODEL != GROQ_MODEL:
            try:
                logger.info(f"Initializing Groq fallback model {GROQ_FALLBACK_MODEL} with max_tokens={GROQ_FALLBACK_MAX_TOKENS}")
                groq_fallback = ChatOpenAI(
                    model=GROQ_FALLBACK_MODEL,
                    openai_api_key=GROQ_API_KEY,
                    openai_api_base="https://api.groq.com/openai/v1",
                    temperature=temperature,
                    max_tokens=GROQ_FALLBACK_MAX_TOKENS,
                    default_headers={
                        "HTTP-Referer": "http://localhost:8000",
                        "X-Title": "Group Maker"
                    }
                )
                llms.append(groq_fallback)
                logger.info("Groq fallback model added to LLM list")
            except Exception as e:
                logger.error(f"Failed to initialize Groq fallback model {GROQ_FALLBACK_MODEL}: {e}")
    else:
        logger.warning(f"Skipping Groq: GROQ_API_KEY is missing or invalid.")
    ...
    for llm in llms:
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_exception = e
                available = _parse_groq_available_tokens(e)
                if available and available < current_max_tokens:
                    logger.info(f"Groq credit limit detected, retrying with max_tokens={available}")
...
def get_hybrid_crew_llm():
    ...
    has_key = bool(GROQ_API_KEY) and GROQ_API_KEY != "your_groq_api_key_here"
    if has_key:
        crew_model = GROQ_CREW_MODEL
        try:
            logger.info(
                "Configuring CrewAI for Groq (model=%s, max_tokens=%s)",
                crew_model, GROQ_CREW_MAX_TOKENS,
            )
            llm = LLM(
                ...
                max_completion_tokens=GROQ_CREW_MAX_TOKENS,
            )
            return llm
        except Exception as e:
            logger.warning(f"Groq crew LLM init failed: {e}. Falling back to Ollama.")
